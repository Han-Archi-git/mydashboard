...
- **`app.js`/`style.css`를 수정해서 푸시했는데 새로고침해도 안 바뀜**: GitHub Pages는 정적 파일을 10분(`max-age=600`) 캐시함. `index.html`의 `app.js?v=N` / `style.css?v=N` 쿼리 버전을 올리면 캐시 무시하고 새로 받아감 — **app.js나 style.css를 수정할 때마다 `index.html`의 두 `?v=` 숫자를 같이 올릴 것**. 급하면 강제 새로고침(Ctrl+Shift+R / Cmd+Shift+R)으로도 확인 가능.
